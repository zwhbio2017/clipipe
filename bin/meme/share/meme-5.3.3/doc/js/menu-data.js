...
MemeMenu.blurbs = {
  ...
  "streme": "<dfn>STREME</dfn> discovers <em>ungapped</em> motifs (recurring, fixed-length patterns) that are <em>enriched</em> in your sequences or <em>relatively enriched</em> in them compared to your control sequences<span class=\"sample_output\"> (<a href=\"../doc/examples/streme_example_output_files/streme.html\">sample output</a> from <a href='https://meme-suite.org/meme/doc/examples/example-datasets/Klf1.fna'>sequences</a>)</span>.<span class=\"manual_link\"> See this <a href=\"../doc/streme.html?man_type=web\">Manual</a> or this <a href=\"../doc/streme-tutorial.html\">Tutorial</a> for more information.</span>  <footer><br><br>STREME (Simple, Thorough, Rapid, Enriched Motif Elicitation) finds motifs (up to 30 columns wide) in large sequence datasets. The input to STREME is one or two sets of sequences.  The control sequences should have approximately the same length distribution as the primary sequences. If you do not provide a control set, the program shuffles the primary set to create a control set. The program uses Fisher's Exact Test or the Binomial test to determine significance of each motif found in the positive set as compared with its representation in the control set, using a significance threshold that may be set on the command line.<br><br>STREME achieves high speed via an optimization algorithm that uses a suffix tree, and provides <b>accurate estimates</b> of the statistical significance of the motifs it discovers. <br><br><b>Note:</b> If you have fewer than 50 sequences, you might want to use MEME instead of STREME. <br><br><b>Note:</b> STREME will not include sequence positions that contain ambiguous characters in the motifs it discovers.</footer>",

  "enr": "<dfn>ENR</dfn> identifies <em>known user-provided</em> motifs that are <em>relatively</em> enriched in your sequences compared with shuffled sequences or your control sequences<span class=\"sample_output\"> (<a href=\"../doc/examples/enr_example_output_files/enr.html\">sample output</a> from <a href='https://meme-suite.org/meme/doc/examples/example-datasets/Klf1.fna'>sequences</a>)</span>.<span class=\"manual_link\"> See this <a href=\"../doc/enr.html?man_type=web\">Manual</a> for more information.</span>  <footer><br><br>ENR (Motif ENRichment Analysis) applies the same objective function used by the STREME motif discovery algorithm to measure the enrichment of motifs large sequence datasets. It is useful for comparing the discriminative power of motifs found by STREME and other motif discovery tools. The input to ENR is one or two sets of sequences.  The control sequences should have approximately the same length distribution as the primary sequences. If you do not provide a control set, the program shuffles the primary set to create a control set. The program uses Fisher's Exact Test to determine significance of each motif found in the positive set as compared with its representation in the control set, using a significance threshold that may be set on the command line.<br><br>ENR achieves high speed via an optimization algorithm that uses a suffix tree.  <br><br><b>Note:</b> ENR ignores sequence positions that contain ambiguous characters.</footer>",
  ...
  "topics": [
    {
      ...
      "topics": [
        {
          ...
          "info": "Tomtom produces an HTML results file, a Gene Targets TSV file, a Gene Elements TSV file, a TSS Targets TSV file, a TSS Elements TSV file and a Links TSV file.",
      ...
      "topics": [
        ...
        },
        {
          ...
          "url": "https://meme-suite.org/meme/opal2/dashboard?command=statistics",
          "absolute": true
        },
        {
