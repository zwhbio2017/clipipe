...
use lib qw(/home/CLIPipe_user/meme/lib/meme-5.3.3/perl);
...
unless ($@) {
  Log::Log4perl::init('/home/CLIPipe_user/meme/share/meme-5.3.3/logging.conf');
  $logger = Log::Log4perl->get_logger('meme.service.utils');
}

my $service_invocation_log_dir = '/home/CLIPipe_user/meme/var/meme-5.3.3/LOGS';
my $tmpdir = '';
# use the perl default if none is supplied or the replace fails
$tmpdir = &tmpdir() if ($tmpdir eq '' || $tmpdir =~ m/^\@TMP[_]DIR\@$/);
my @gs_version_nums = ();
my $GHOSTSCRIPT = '/home/CLIPipe_user/bin/miniconda3/envs/clipipe/bin/gs';
my $CONVERT = '';
##############################################################################
#          Functions
##############################################################################
...
sub update_status {
  ...
  my $template = HTML::Template->new(filename => '/home/CLIPipe_user/meme/share/meme-5.3.3/job_status.tmpl');
